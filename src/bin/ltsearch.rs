//! 单二进制入口（#124）：`ltsearch <write|build|query|static-build|static-activate> [args…]`。
...
const USAGE: &str = "usage: ltsearch <write|build|query|static-build|static-activate> [args...]\n\
...
fn main() -> ExitCode {
    ...
    let result: Result<(), Box<dyn std::error::Error>> = match subcommand.as_deref() {
        ...
        Some("static-activate") => runtime
            .block_on(ltsearch::app::run_static_activate(
                rest.iter().map(String::as_str),
            ))
            .map(|summary| println!("{summary}")),
        Some(other) => {
